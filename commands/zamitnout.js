...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('zamitnout')
        .setDescription('Zamítnout nápad')
    ...
    async execute(interaction) {
        ...
        const result = await updateIdeaStatus(channel, messageId, 'zamitnuto', interaction.user);

        if (result) {
            await interaction.reply({ content: `❌ Nápad **${messageId}** byl zamítnut!`, ephemeral: true });
        } else {
